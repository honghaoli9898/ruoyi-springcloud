...
public class BpmTaskServiceImpl implements BpmTaskService {
	...
	public PageResult<BpmTaskDonePageItemRespVO> getDoneTaskPage(Long userId,
		...
		HistoricTaskInstanceQuery taskQuery = historyService
				.createHistoricTaskInstanceQuery().finished() // 已完成
				.taskAssignee(String.valueOf(userId)) // 分配给自己
	...
	public void updateTaskExtComplete(Task task) {
		BpmTaskExtDO taskExtDO = BpmTaskConvert.INSTANCE.convert2TaskExt(task);
		taskExtDO.setResult(BpmProcessInstanceResultEnum.APPROVE.getResult()); // 不设置也问题不大，因为
																				// Complete
																				// 一般是审核通过，已经设置
		...
		TransactionSynchronizationManager
				.registerSynchronization(new TransactionSynchronization() {
					...
					public void afterCommit() {
						...
						if (taskExt == null) {
							log.error(
									"[updateTaskExtCancel][taskId({}) 查找不到对应的记录，可能存在问题]",
									taskId);
							return;
						}
						// 如果已经是最终的结果，则跳过
