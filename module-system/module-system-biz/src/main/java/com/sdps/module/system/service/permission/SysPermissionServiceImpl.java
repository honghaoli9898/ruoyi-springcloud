...
public class SysPermissionServiceImpl implements SysPermissionService {
	...
	/**
	 * 角色编号与菜单编号的缓存映射 key：角色编号 value：菜单编号的数组
	 *
	...
	/**
	 * 菜单编号与角色编号的缓存映射 key：菜单编号 value：角色编号的数组
	 *
	 * 这里声明 volatile 修饰的原因是，每次刷新时，直接修改指向
	 */
	@Getter
	...
	/**
	 * 用户编号与角色编号的缓存映射 key：用户编号 value：角色编号的数组
	 *
	 * 这里声明 volatile 修饰的原因是，每次刷新时，直接修改指向
	 */
	@Getter
	...
	void initUserRoleLocalCache() {
		...
		log.info("[initUserRoleLocalCache][初始化用户与角色的关联数量为 {}]",
				userRoleList.size());
	}

	/**
	 * 如果用户与角色的关联发生变化，从数据库中获取最新的全量用户与角色的关联。 如果未发生变化，则返回空
	 *
	 * @param maxUpdateTime
	 *            当前角色与菜单的关联的最大更新时间
	 * @return 角色与菜单的关联列表
	...
	void initRoleMenuLocalCache() {
		...
		log.info("[initRoleMenuLocalCache][初始化角色与菜单的关联数量为 {}]",
				roleMenuList.size());
	}

	/**
	 * 如果角色与菜单的关联发生变化，从数据库中获取最新的全量角色与菜单的关联。 如果未发生变化，则返回空
	 *
	 * @param maxUpdateTime
	 *            当前角色与菜单的关联的最大更新时间
	 * @return 角色与菜单的关联列表
	...
	// 关闭数据权限，不然就会出现递归获取数据权限的问题
	@TenantIgnore
	// 忽略多租户的自动过滤。如果不忽略，会导致添加租户时，因为切换租户，导致获取不到 User。即使忽略，本身该方法不存在跨租户的操作，不会存在问题。
