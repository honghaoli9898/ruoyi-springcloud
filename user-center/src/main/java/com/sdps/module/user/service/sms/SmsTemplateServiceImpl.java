...
public class SmsTemplateServiceImpl implements SmsTemplateService {
	...
	@Resource
	private SmsTemplateMapper sysSmsTemplateMapper;

	/**
	 * 短信模板缓存 key：短信模板编码 {@link SmsTemplateDO#getCode()}
	 *
	...
	public void initLocalCache() {
		// 获取短信模板列表，如果有更新
		List<SmsTemplateDO> smsTemplateList = this
				.loadSmsTemplateIfUpdate(maxUpdateTime);
		if (CollUtil.isEmpty(smsTemplateList)) {
			return;
		}

		// 写入缓存
		smsTemplateCache = CollectionUtils.convertMap(smsTemplateList,
				SmsTemplateDO::getCode);
		maxUpdateTime = CollectionUtils.getMaxValue(smsTemplateList,
				SmsTemplateDO::getUpdateTime);
		log.info("[initLocalCache][初始化 SmsTemplate 数量为 {}]",
				smsTemplateList.size());
	}

	/**
	 * 如果短信模板发生变化，从数据库中获取最新的全量短信模板。 如果未发生变化，则返回空
	 *
	 * @param maxUpdateTime
	 *            当前短信模板的最大更新时间
	 * @return 短信模板列表
	...
	@Override
	public String formatSmsTemplateContent(String content,
	...
	public Long createSmsTemplate(SmsTemplateCreateReqVO createReqVO) {
		...
		// 校验短信模板
		checkApiTemplate(createReqVO.getChannelId(),
				createReqVO.getApiTemplateId());

		// 插入
		SmsTemplateDO template = SmsTemplateConvert.INSTANCE
				.convert(createReqVO);
	...
	public void updateSmsTemplate(SmsTemplateUpdateReqVO updateReqVO) {
		...
		// 校验短信编码是否重复
		checkSmsTemplateCodeDuplicate(updateReqVO.getId(),
				updateReqVO.getCode());
		// 校验短信模板
		checkApiTemplate(updateReqVO.getChannelId(),
				updateReqVO.getApiTemplateId());

		// 更新
		SmsTemplateDO updateObj = SmsTemplateConvert.INSTANCE
				.convert(updateReqVO);
	...
	private void validateSmsTemplateExists(Long id) {
		if (smsTemplateMapper.selectById(id) == null) {
			throw ServiceExceptionUtil
					.exception(ErrorCodeConstants.SMS_TEMPLATE_NOT_EXISTS);
		}
	}
	...
	@Override
	public PageResult<SmsTemplateDO> getSmsTemplatePage(
			SmsTemplatePageReqVO pageReqVO) {
		return smsTemplateMapper.selectPage(pageReqVO);
	}

	@Override
	public List<SmsTemplateDO> getSmsTemplateList(
			SmsTemplateExportReqVO exportReqVO) {
	...
	public SmsChannelDO checkSmsChannel(Long channelId) {
		...
		if (channelDO == null) {
			throw ServiceExceptionUtil
					.exception(ErrorCodeConstants.SMS_CHANNEL_NOT_EXISTS);
		}
		if (!Objects.equals(channelDO.getStatus(),
				CommonStatusEnum.ENABLE.getStatus())) {
			throw ServiceExceptionUtil
					.exception(ErrorCodeConstants.SMS_CHANNEL_DISABLE);
		}
		return channelDO;
	}
	...
	public void checkSmsTemplateCodeDuplicate(Long id, String code) {
		...
		if (id == null) {
			throw ServiceExceptionUtil.exception(
					ErrorCodeConstants.SMS_TEMPLATE_CODE_DUPLICATE, code);
		}
	...
	public void checkApiTemplate(Long channelId, String apiTemplateId) {
		...
		SmsCommonResult<SmsTemplateRespDTO> templateResult = smsClient
