...
public class SmsChannelServiceImpl implements SmsChannelService {
	...
	public void initSmsClients() {
		// 获取短信渠道，如果有更新
		List<SmsChannelDO> smsChannels = this
				.loadSmsChannelIfUpdate(maxUpdateTime);
		if (CollUtil.isEmpty(smsChannels)) {
			return;
		}

		// 创建或更新短信 Client
		List<SmsChannelProperties> propertiesList = SmsChannelConvert.INSTANCE
				.convertList02(smsChannels);
		propertiesList.forEach(properties -> smsClientFactory
				.createOrUpdateSmsClient(properties));

		// 写入缓存
		maxUpdateTime = CollectionUtils.getMaxValue(smsChannels,
				SmsChannelDO::getUpdateTime);
	...
	/**
	 * 如果短信渠道发生变化，从数据库中获取最新的全量短信渠道。 如果未发生变化，则返回空
	 *
	 * @param maxUpdateTime
	 *            当前短信渠道的最大更新时间
	 * @return 短信渠道列表
	...
	public Long createSmsChannel(SmsChannelCreateReqVO createReqVO) {
		// 插入
		SmsChannelDO smsChannel = SmsChannelConvert.INSTANCE
				.convert(createReqVO);
	...
	public void updateSmsChannel(SmsChannelUpdateReqVO updateReqVO) {
		...
		// 更新
		SmsChannelDO updateObj = SmsChannelConvert.INSTANCE
				.convert(updateReqVO);
	...
	public void deleteSmsChannel(Long id) {
		...
		if (smsTemplateService.countByChannelId(id) > 0) {
			throw ServiceExceptionUtil
					.exception(ErrorCodeConstants.SMS_CHANNEL_HAS_CHILDREN);
		}
		// 删除
