...
public class ValidateCodeServiceImpl implements IValidateCodeService {
	...
	public void saveImageCode(String deviceId, String imageCode) {
		redisRepository.setExpire(buildKey(deviceId), imageCode,
				SecurityConstants.DEFAULT_IMAGE_EXPIRE);
	}
	...
	public CommonResult sendSmsCode(String mobile) {
		...
		if (tempCode != null) {
			...
			return CommonResult.error(1, "验证码未失效，请失效后再次申请");
		}

		AdminUserDO user = userService.findByMobile(mobile);
		if (user == null) {
			log.error("根据用户手机号{}查询用户为空", mobile);
			return CommonResult.error(1, "手机号不存在");
		}

		String code = RandomUtil.randomNumbers(4);
		log.info("短信发送请求消息中心 -> 手机号:{} -> 验证码：{}", mobile, code);
		redisRepository.setExpire(buildKey(mobile), code,
				SecurityConstants.DEFAULT_IMAGE_EXPIRE);
