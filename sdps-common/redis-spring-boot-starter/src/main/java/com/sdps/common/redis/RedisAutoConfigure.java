package com.sdps.common.redis;

import com.sdps.common.redis.properties.CacheManagerProperties;
...
public class RedisAutoConfigure {
	...
	public CacheManager cacheManager(
			...
			RedisSerializer<String> redisKeySerializer,
			RedisSerializer<Object> redisValueSerializer) {
		RedisCacheConfiguration difConf = getDefConf(redisKeySerializer,
				redisValueSerializer).entryTtl(Duration.ofHours(1));
		difConf = difConf
		...
		Map<String, RedisCacheConfiguration> redisCacheConfigurationMap = new HashMap<>(
				configSize);
		if (configSize > 0) {
			cacheManagerProperties.getConfigs().forEach(
					e -> {
	...
	private RedisCacheConfiguration getDefConf(
		...
		return RedisCacheConfiguration
				...
				.computePrefixWith(
						cacheName -> "cache".concat(":").concat(cacheName)
								.concat(":"))
				.serializeKeysWith(
