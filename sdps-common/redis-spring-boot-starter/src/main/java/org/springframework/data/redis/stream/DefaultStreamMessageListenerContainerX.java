...
public class DefaultStreamMessageListenerContainerX<K, V extends Record<K, ?>> extends DefaultStreamMessageListenerContainer<K, V> {
    ...
    private StreamPollTask<K, V> getReadTaskX(StreamReadRequest<K> streamRequest, StreamListener<K, V> listener) {
        ...
        // 修改 readFunction 方法
        Object obj = ReflectUtil.getFieldValue(task, "readFunction");
        if (obj instanceof BiFunction) {
            BiFunction<String, ReadOffset, List<ByteRecord>> readFunction = (BiFunction<String, ReadOffset, List<ByteRecord>>) ReflectUtil.getFieldValue(task, "readFunction");
            ReflectUtil.setFieldValue(task, "readFunction", (BiFunction<String,ReadOffset, List<ByteRecord>>) (key,readOffset) -> {
                List<ByteRecord> records = readFunction.apply(key,readOffset);
