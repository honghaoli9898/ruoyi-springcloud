package com.sdps.common.apilog.core.filter;
...
import com.sdps.common.apilog.core.service.ApiAccessLog;
...
import com.sdps.common.web.core.filter.ApiRequestFilter;
...
public class ApiAccessLogFilter extends ApiRequestFilter {
	...
	protected void doFilterInternal(HttpServletRequest request,
		...
		String requestBody = ServletUtils.isJsonRequest(request) ? ServletUtil
		...
		try {
			...
			// 正常执行，记录日志
			createApiAccessLog(request, beginTim, queryString, requestBody,
					null);
		} catch (Exception ex) {
			// 异常执行，记录日志
	...
	private void createApiAccessLog(HttpServletRequest request, Date beginTime,
		...
		try {
			this.buildApiAccessLogDTO(accessLog, request, beginTime,
	...
	private void buildApiAccessLogDTO(ApiAccessLog accessLog,
			HttpServletRequest request, Date beginTime,
			Map<String, String> queryString, String requestBody, Exception ex) {
		// 处理用户信息
		...
		if (result != null) {
			...
			accessLog
					.setResultCode(GlobalErrorCodeConstants.INTERNAL_SERVER_ERROR
							.getCode());
		...
		Map<String, Object> requestParams = MapUtil.<String, Object> builder()
				.put("query", queryString).put("body", requestBody).build();
		...
		accessLog.setDuration((int) DateUtils.diff(accessLog.getEndTime(),
				accessLog.getBeginTime()));
		Long userId = WebFrameworkUtils.getLoginUserId();
		accessLog.setCreator(Objects.nonNull(userId)?userId.toString():"-1");
	}
