package com.sdps.common.sms.core.client.impl.yunpian;
...
import com.sdps.common.sms.core.client.impl.AbstractSmsClient;
import com.sdps.common.sms.core.enums.SmsTemplateAuditStatusEnum;
...
import static com.sdps.common.util.date.DateUtils.FORMAT_YEAR_MONTH_DAY_HOUR_MINUTE_SECOND;
import static com.sdps.common.util.date.DateUtils.TIME_ZONE_DEFAULT;
...
public class YunpianSmsClient extends AbstractSmsClient {
    ...
    protected SmsCommonResult<SmsSendRespDTO> doSendSms(Long sendLogId, String mobile,
        ...
        return invoke(() -> {
            ...
            smsSendRespDTO.setSerialNo(String.valueOf(response.getSid()));
    ...
    protected List<SmsReceiveRespDTO> doParseSmsReceiveStatus(String text) throws Throwable {
        ...
        return statuses.stream().map(status -> {
            ...
            resp.setErrorCode(status.getErrorMsg());
            resp.setErrorMsg(status.getErrorDetail());
            resp.setMobile(status.getMobile());
            resp.setReceiveTime(status.getUserReceiveTime());
            resp.setSerialNo(String.valueOf(status.getSid()));
            resp.setLogId(status.getUid());
            return resp;
    ...
    protected SmsCommonResult<SmsTemplateRespDTO> doGetSmsTemplate(String apiTemplateId) throws Throwable {
        return invoke(() -> {
            ...
            smsTemplateRespDTO.setId(String.valueOf(template.getTpl_id()));
            smsTemplateRespDTO.setContent(template.getTpl_content());
            smsTemplateRespDTO.setAuditStatus(convertSmsTemplateAuditStatus(template.getCheck_status()));
            smsTemplateRespDTO.setAuditReason(template.getReason());
