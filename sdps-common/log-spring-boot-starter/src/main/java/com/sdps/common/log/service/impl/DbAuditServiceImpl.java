package com.sdps.common.log.service.impl;
...
import com.sdps.common.log.service.IAuditService;
...
public class DbAuditServiceImpl implements IAuditService {
	...
	public DbAuditServiceImpl(
			@Autowired(required = false) LogDbProperties logDbProperties,
			DataSource dataSource) {
		// 优先使用配置的日志数据源，否则使用默认的数据源
		if (logDbProperties != null
				&& StrUtil.isNotEmpty(logDbProperties.getJdbcUrl())) {
	...
	public void save(Audit audit) {
		this.jdbcTemplate.update(INSERT_SQL, audit.getApplicationName(),
				audit.getClassName(), audit.getMethodName(), audit.getUserId(),
				audit.getUserName(), audit.getClientId(), audit.getOperation(),
				audit.getTimestamp());
	}
}
