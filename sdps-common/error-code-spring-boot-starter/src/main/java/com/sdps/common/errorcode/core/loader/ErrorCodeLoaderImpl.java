...
public class ErrorCodeLoaderImpl implements ErrorCodeLoader {
	...
	private void loadErrorCodes0() {
		// 加载错误码
		List<ErrorCodeRespDTO> errorCodeRespDTOs = errorCodeApi
				.getErrorCodeList(applicationName, maxUpdateTime);
		...
		errorCodeRespDTOs.forEach(errorCodeRespDTO -> {
			// 写入到错误码的缓存
				putErrorCode(errorCodeRespDTO.getCode(),
						errorCodeRespDTO.getMessage());
				// 记录下更新时间，方便增量更新
				maxUpdateTime = DateUtils.max(maxUpdateTime,
						errorCodeRespDTO.getUpdateTime());
			});
	}
