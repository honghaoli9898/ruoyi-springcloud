package com.sdps.common.errorcode.core.generator;
...
public class ErrorCodeAutoGeneratorImpl implements ErrorCodeAutoGenerator {
	...
	private List<ErrorCodeAutoGenerateReqDTO> parseErrorCode() {
		...
		constantsClassList.forEach(constantsClass -> {
			try {
				// 解析错误码枚举类
				Class<?> errorCodeConstantsClazz = ClassUtil
						.loadClass(constantsClass);
				// 解析错误码
				autoGenerateDTOs
						.addAll(parseErrorCode(errorCodeConstantsClazz));
			} catch (Exception ex) {
				log.warn("[parseErrorCode][constantsClass({}) 加载失败({})]",
						constantsClass, ExceptionUtil.getRootCauseMessage(ex));
			}
		});
	...
	private List<ErrorCodeAutoGenerateReqDTO> parseErrorCode(
		...
		Arrays.stream(constantsClass.getFields()).forEach(field -> {
			...
			// 转换成 ErrorCodeAutoGenerateReqDTO 对象
				ErrorCode errorCode = (ErrorCode) ReflectUtil.getFieldValue(
						constantsClass, field);
				autoGenerateDTOs.add(new ErrorCodeAutoGenerateReqDTO()
